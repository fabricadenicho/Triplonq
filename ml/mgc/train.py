"""
Treina XGBoost para prever direcao do MGC (LONG/NEUTRO/SHORT).
Feature set adaptado: MGC como ativo primario, MNQ/BTC como secundarios.
...
def build_features(conn, interval='1h', forward=8, min_ret=0.001):
    mgc = compute(load_symbol(conn, 'mgc', interval))
    ...
    # RSI dos 3 ativos (sufixo _cl = slot do ativo primario)
    f['rsi_cl']  = mgc['rsi']
    ...
    primary_high  = mgc['high']
    primary_low   = mgc['low']
    primary_close = mgc['close']
    ...
    add_key_levels(mgc, f)

    future_price   = mgc['close'].shift(-forward)
    f['future_ret'] = future_price / mgc['close'] - 1
    f['label']      = 1
...
def plot_importance(model):
    ...
    ax.set_title('Feature Importance (Gain) - MGC XGBoost')
    plt.tight_layout()
...
def main():
    ...
    p.add_argument('--all-hours',    action='store_true', help='Treinar em todas as horas')
    ...
    p.add_argument('--kill-zone', type=str, default=None, choices=['asia', 'london', 'ny'],
                   help='Treinar apenas para uma kill zone especifica')
    args = p.parse_args()
    ...
    if not args.all_hours:
        ...
        print('Modo todas as horas ativo')
