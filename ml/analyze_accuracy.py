"""
Analise pratica do modelo: acertividade por threshold, sessao, regime.
"""
...
print(f'\n{"="*60}')
print(f'  ACERTIVIDADE PRATICA - Sessao US')
print(f'{"="*60}')
...
# 2. Acertividade por threshold de confianca
print(f'\n{"─"*60}')
...
# 3. Acertividade combinada (prob_long - prob_short)
print(f'\n{"─"*60}')
...
# 4. Gatilho us_prime_setup com prob
print(f'\n{"─"*60}')
...
# 6. EMA50 bias regime
print(f'\n{"─"*60}')
print(f'  6. VIÉS EMA50 + CONFIANCA')
print(f'{"─"*60}')
for bias, lbl in [(0, 'ambos abaixo'), (1, 'misturado'), (2, 'ambos acima')]:
    sub = df_te[df_te['ema50_bias_mnq_btc'] == bias]
    if len(sub) < 10: continue
    long_actual = (sub['label'] == 2).mean()
    short_actual = (sub['label'] == 0).mean()
    pred_correct = (sub['pred'] == sub['label']).mean()
...
# 7. Resumo de acertividade por sinal
print(f'\n{"─"*60}')
print(f'  RESUMO - QUANDO O MODELO DA SINAL DE LONGSHORT')
print(f'{"─"*60}')
...
# Acertou pelo menos a direcao (LONG ou SHORT, ignorando NEUTRO)
...
# Acertou ou errou por pouco (NEUTRO quando era LONG/SHORT)
