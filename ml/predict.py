...
def main():
    ...
    try:
        ...
        r['sma50_slope_mnq']= last(mnq, 'sma50_slope') or 0.0
        ...
        # ── Kill zone features (computado em toda a serie para session tracking) ──
        ASIA_HOURS   = [0, 1, 2, 3, 4, 5, 6, 7]
        LONDON_HOURS = [8, 9, 10, 11, 12, 13, 14]
        NY_HOURS     = [13, 14, 15, 16, 17, 18, 19]
        OVERLAP_HOURS = [13, 14]

        mnq_hour = mnq.index.hour
        in_asia   = pd.Series(mnq_hour.isin(ASIA_HOURS), index=mnq.index)
        in_london = pd.Series(mnq_hour.isin(LONDON_HOURS), index=mnq.index)
        in_ny     = pd.Series(mnq_hour.isin(NY_HOURS), index=mnq.index)
        ...
        session_high = mnq['high'].groupby(kz_session_id).cummax()
        session_low  = mnq['low'].groupby(kz_session_id).cummin()
        ...
        r['kz_dist_high'] = float(((mnq['close'] - session_high) / session_high * 100).iloc[-1]) if in_any.iloc[-1] else 0.0
        r['kz_dist_low']  = float(((mnq['close'] - session_low) / session_low * 100).iloc[-1]) if in_any.iloc[-1] else 0.0
        ...
        proba = model.predict_proba(X)[0]  # [prob_SHORT, prob_NEUTRO, prob_LONG]
        pred  = int(model.predict(X)[0])    # 0=SHORT, 1=NEUTRO, 2=LONG
        ...
        if current_kz:
            ...
            if kz_path.exists():
                ...
                w_main, w_kz = 0.4, 0.6
                proba = w_main * proba + w_kz * proba_kz
