"""
Backtest completo — Estrategia S1: ROT-LONG + acima 4H open + ML
Split temporal: 80% treino | 10% validacao OOS | 10% OOS cego

Avalia 3 versoes:
  1. Baseline   : qualquer barra
  2. S1 sem ML  : rot_score>=60 + div_cl>0 + close>4H_open
  3. S1 com ML  : S1 sem ML + ml_prob>=threshold

Uso:
  python backtest_s1_4h.py [--forward 4] [--period 730] [--thr 0.50] [--save-trades]
"""
import argparse, warnings, pickle, csv
...
# Download
# ─────────────────────────────────────────────────────────────────────────────
def download_asset(ticker: str, period_days: int) -> pd.DataFrame | None:
    import time
    end   = datetime.now()
...
# ─────────────────────────────────────────────────────────────────────────────
# Features — espelho exato de predict_s1_4h.py
# ─────────────────────────────────────────────────────────────────────────────
def build_features(raw: dict, forward: int = 4) -> pd.DataFrame:
    ...
    # 4H open do MNQ (para filtro above_4h_open)
    f['open_4h_mnq'] = (
    ...
    if removed > 0:
        print(f'  Rollover filter: {removed} barras removidas (|ret1h| > {ROLLOVER_THRESHOLD*100:.0f}%)')

    return f


# ─────────────────────────────────────────────────────────────────────────────
# Rot score vetorizado — espelho exato da logica de predict_s1_4h.py
# ─────────────────────────────────────────────────────────────────────────────
def compute_rot_score(f: pd.DataFrame) -> pd.Series:
    ...
    rot_c4     = f['es_mnq_mesmo'].astype(float)

    rot_c5     = (f['bb_w_cl'] > 1.5).astype(float)
    rot_c6     = f['open_d_acima_d_ant_cl'].astype(float)
...
# ─────────────────────────────────────────────────────────────────────────────
# Metricas de um split
# ─────────────────────────────────────────────────────────────────────────────
def _pct(x): return f'{x:.1%}'

def eval_split(label: str, fslice: pd.DataFrame, probs: pd.Series | None = None,
               ml_thr: float = 0.50, save_trades: bool = False,
               trades_path: Path | None = None) -> dict:
    n          = len(fslice)
    baseline   = float(fslice['target'].mean())

    mask_sem   = (
        (fslice['rot_score'] >= 60) &
        (fslice['div_cl']    >  0 ) &
        (fslice['above_4h_open'] == 1)
    )
    ...
    print(f'  |  S1 sem ML  N={n_sem:>4}  WR={_pct(wr_sem)}  Edge={wr_sem-baseline:+.1%}')

    result = {
        'label': label, 'n': n, 'baseline': baseline,
        'n_s1_sem': n_sem, 'wr_s1_sem': wr_sem,
    }
    ...
    if probs is not None:
        ...
        alpha    = wr_com - wr_sem
        result.update({'n_s1_com': n_com, 'wr_s1_com': wr_com, 'alpha': alpha})

        print(f'  |  S1 com ML  N={n_com:>4}  WR={_pct(wr_com)}  '
        ...
        if save_trades and trades_path is not None:
            trades = fslice.loc[mask_com, [
                'mnq','div_cl','rot_score','above_4h_open','target','hour','dow'
            ]].copy()
...
# ─────────────────────────────────────────────────────────────────────────────
def main():
    p = argparse.ArgumentParser(description='Backtest S1 — ROT-LONG + 4H open + ML')
    p.add_argument('--forward',     type=int,   default=4,    help='Horas forward (default=4)')
    p.add_argument('--period',      type=int,   default=720,  help='Dias de historico (default=720)')
    p.add_argument('--thr',         type=float, default=0.50, help='ML threshold (default=0.50)')
    p.add_argument('--save-trades', action='store_true',      help='Salva trades em CSV')
    args = p.parse_args()

    trades_path = BASE / 'backtest_s1_trades.csv' if args.save_trades else None
    if trades_path and trades_path.exists():
        trades_path.unlink()

    print('=' * 62)
    print(' BACKTEST S1 — ROT-LONG + acima 4H open + ML')
    print('=' * 62)
    print(f' Periodo   : {args.period} dias de historico (1h bars)')
    ...
    print(f'Target=1 (subiu): {f["target"].mean():.1%}  |  Baseline WR: {f["target"].mean():.1%}')

    # ── Rot score ───────────────────────────────────────────────────────────
    print('\nCalculando rot_score (vetorizado)...')
    f['rot_score']    = compute_rot_score(f)
    f['above_4h_open'] = (f['mnq'] > f['open_4h_mnq']).astype(int)
    f = f.dropna(subset=['rot_score']).copy()

    if len(f) < 300:
        print('Poucos dados. Abortando.'); return

    rot_fires = int((f['rot_score'] >= 60).sum())
    s1_sem_n  = int(((f['rot_score'] >= 60) & (f['div_cl'] > 0) & (f['above_4h_open'] == 1)).sum())
    print(f'rot_score >= 60: {rot_fires} barras ({rot_fires/len(f):.1%})')
    print(f'S1 sem ML total: {s1_sem_n} sinais')

    # ── Split 80 / 10 / 10 ──────────────────────────────────────────────────
    n      = len(f)
    n_tr   = int(n * 0.80)
    n_val  = int(n * 0.10)
    # OOS = restante (pode ser levemente > 10% por arredondamento)
    ...
    print(f'\nSplits:')
    print(f'  Treino (80%)    : {len(f_tr)} barras  '
          f'{f_tr.index[0].date()} ->{f_tr.index[-1].date()}')
    print(f'  Validacao (10%) : {len(f_val)} barras  '
          f'{f_val.index[0].date()} ->{f_val.index[-1].date()}')
    print(f'  OOS cego (10%)  : {len(f_oos)} barras  '
          f'{f_oos.index[0].date()} ->{f_oos.index[-1].date()}')
    ...
    SKIP = {
        ...
        'open_4h_mnq', 'rot_score', 'above_4h_open',
    }
    ...
    # Treino interno: 80% fit + 20% early-stopping
    ...
    # ── Treino XGBoost ───────────────────────────────────────────────────────
    print(f'\nTreinando XGBoost ({len(feat_cols)} features)...')
    ...
    auc_tr  = roc_auc_score(y_tr,  prob_tr)  if len(y_tr)  > 10 else 0.0
    auc_val = roc_auc_score(y_val, prob_val) if len(y_val) > 10 else 0.0
    auc_oos = roc_auc_score(y_oos, prob_oos) if len(y_oos) > 10 else 0.0
    ...
    r_tr  = eval_split('TREINO (80% in-sample)', f_tr,  prob_tr,
                       ml_thr=args.thr, save_trades=args.save_trades,
    ...
    wr_sem_oos = r_oos['wr_s1_sem']
    wr_com_oos = r_oos.get('wr_s1_com', 0.0)
    n_sem_oos  = r_oos['n_s1_sem']
    n_com_oos  = r_oos.get('n_s1_com', 0)

    print(f'  Baseline           : {_pct(base_oos)}')
    print(f'  S1 sem ML  N={n_sem_oos:>4}  WR={_pct(wr_sem_oos)}  Edge={wr_sem_oos - base_oos:+.1%}')
    print(f'  S1 com ML  N={n_com_oos:>4}  WR={_pct(wr_com_oos)}  Edge={wr_com_oos - base_oos:+.1%}  '
          f'Alpha={wr_com_oos - wr_sem_oos:+.1%}')
    print(f'  AUC OOS            : {auc_oos:.4f}')

    # Drift check treino ->OOS
    if 'wr_s1_com' in r_tr and 'wr_s1_com' in r_oos:
        drift = r_tr['wr_s1_com'] - r_oos['wr_s1_com']
        status = 'OK' if abs(drift) < 0.15 else 'ATENCAO: possivel overfitting'
        print(f'  Drift treino->OOS  : {drift:+.1%}  [{status}]')

    # ── Salvar modelo + artefatos ────────────────────────────────────────────
    model_out = BASE / 'model_s1_4h.pkl'
    with open(model_out, 'wb') as fh:
        pickle.dump({
            ...
            'source':    'yfinance',
            'split':     '80/10/10',
            'wr_s1_sem_oos': wr_sem_oos,
            'wr_s1_com_oos': wr_com_oos,
        }, fh)
