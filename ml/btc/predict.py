...
def main():
    ...
    try:
        ...
        btc_hour = btc.index.hour
        in_asia   = pd.Series(btc_hour.isin(ASIA_HOURS), index=btc.index)
        in_london = pd.Series(btc_hour.isin(LONDON_HOURS), index=btc.index)
        in_ny     = pd.Series(btc_hour.isin(NY_HOURS), index=btc.index)
        ...
        session_high = btc['high'].groupby(kz_session_id).cummax()
        session_low  = btc['low'].groupby(kz_session_id).cummin()
        ...
        r['kz_dist_high'] = float(((btc['close'] - session_high) / session_high * 100).iloc[-1]) if in_any.iloc[-1] else 0.0
        r['kz_dist_low']  = float(((btc['close'] - session_low) / session_low * 100).iloc[-1]) if in_any.iloc[-1] else 0.0
        ...
        r.update(compute_key_levels_live(btc_long if btc_long is not None else btc_df))
