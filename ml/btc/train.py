"""
Treina XGBoost para prever direcao do BTC (LONG/NEUTRO/SHORT).
...
def build_features(conn, interval='1h', forward=8, min_ret=0.001):
    ...
    # RSI dos 3 ativos
    f['rsi_mnq'] = btc['rsi']
    f['rsi_btc'] = mnq['rsi'].reindex(idx, method='ffill')
    f['rsi_cl']  = cl['rsi'].reindex(idx, method='ffill')

    # ADX + DI
    f['adx_mnq'] = btc['adx']
    f['adx_btc'] = mnq['adx'].reindex(idx, method='ffill')
    f['adx_cl']  = cl['adx'].reindex(idx, method='ffill')
    f['pdi_mnq'] = btc['pdi']
    f['mdi_mnq'] = btc['mdi']

    # Divergencia RSI (BTC-centric)
    f['div_cl']  = f['rsi_mnq'] - f['rsi_cl']
    f['div_btc'] = f['rsi_mnq'] - f['rsi_btc']

    # Retornos e vol BTC (primary)
    f['ret1_mnq'] = btc['ret1']
    f['ret4_mnq'] = btc['ret4']
    f['ret8_mnq'] = btc['ret8']
    f['vol_mnq']  = btc['vol']
    ...
    # Retornos secondary
    f['ret1_btc'] = mnq['ret1'].reindex(idx, method='ffill')
    f['ret4_btc'] = mnq['ret4'].reindex(idx, method='ffill')
    ...
    # Sinal triplo (mesma formula, mapeamento trocado: _btc = MNQ agora)
    f['triple_signal'] = (
        (f['rsi_btc'] < 45) & (f['div_cl'] < 0) & (f['div_btc'] < 0)
    ).astype(int)

    # MA50 dos 3 ativos
    f['dist_sma50_mnq'] = btc['dist_sma50']
    f['dist_sma50_btc'] = mnq['dist_sma50'].reindex(idx, method='ffill')
    f['dist_sma50_cl']  = cl['dist_sma50'].reindex(idx, method='ffill')
    f['sma50_slope_mnq']= btc['sma50_slope']
    f['above_sma50_mnq']= btc['above_sma50']
    f['above_sma50_btc']= mnq['above_sma50'].reindex(idx, method='ffill')
    ...
    # EMA20 dos 3 ativos
    f['dist_ema20_mnq'] = btc['dist_ema20']
    f['dist_ema20_btc'] = mnq['dist_ema20'].reindex(idx, method='ffill')
    f['dist_ema20_cl']  = cl['dist_ema20'].reindex(idx, method='ffill')
    f['above_ema20_mnq']= btc['above_ema20']
    f['above_ema20_btc']= mnq['above_ema20'].reindex(idx, method='ffill')
    f['above_ema20_cl'] = cl['above_ema20'].reindex(idx, method='ffill')
    f['ema20_bias_mnq_btc'] = f['above_ema20_mnq'] + f['above_ema20_btc']
    ...
    # Retorno differences
    f['ret1_spread_mnq_btc'] = f['ret1_mnq'] - ret_mnq
    f['ret1_spread_mnq_cl']  = f['ret1_mnq'] - ret_cl
    f['ret1_spread_btc_cl']  = ret_mnq - ret_cl
    ret4_mnq = mnq['ret4'].reindex(idx, method='ffill')
    ret4_cl  = cl['ret4'].reindex(idx, method='ffill')
    f['ret4_spread_mnq_btc'] = f['ret4_mnq'] - ret4_mnq
    f['ret4_spread_mnq_cl']  = f['ret4_mnq'] - ret4_cl
    f['ret4_spread_btc_cl']  = ret4_mnq - ret4_cl

    # Co-movement
    f['ret1_prod_mnq_btc'] = f['ret1_mnq'] * ret_mnq
    f['ret1_prod_btc_cl']  = ret_mnq * ret_cl
    f['price_div_cl']      = f['ret1_mnq'] * ret_cl
    f['price_div_abs']     = f['price_div_cl'].abs()
    f['ret4_prod_mnq_btc'] = f['ret4_mnq'] * ret4_mnq
    f['ret4_prod_mnq_cl']  = f['ret4_mnq'] * ret4_cl
    f['ret4_prod_btc_cl']  = ret4_mnq * ret4_cl

    # Volatilidade spreads
    f['vol_spread_mnq_btc'] = f['vol_mnq'] - vol_mnq
    f['vol_spread_mnq_cl']  = f['vol_mnq'] - vol_cl
    f['vol_spread_btc_cl']  = vol_mnq - vol_cl

    # Bollinger spreads
    f['bb_spread_mnq_btc'] = f['bb_mnq'] - bb_mnq
    f['bb_spread_mnq_cl']  = f['bb_mnq'] - bb_cl
    f['bb_spread_btc_cl']  = bb_mnq - bb_cl

    # MA50 distance spreads
    m50_mnq = mnq['dist_sma50'].reindex(idx, method='ffill')
    m50_cl  = cl['dist_sma50'].reindex(idx, method='ffill')
    f['sma50_dist_spread_mnq_btc'] = f['dist_sma50_mnq'] - m50_mnq
    f['sma50_dist_spread_mnq_cl']  = f['dist_sma50_mnq'] - m50_cl
    f['sma50_dist_spread_btc_cl']  = m50_mnq - m50_cl

    # EMA20 distance spreads
    e20_mnq = mnq['dist_ema20'].reindex(idx, method='ffill')
    e20_cl  = cl['dist_ema20'].reindex(idx, method='ffill')
    f['ema20_dist_spread_mnq_btc'] = f['dist_ema20_mnq'] - e20_mnq
    f['ema20_dist_spread_mnq_cl']  = f['dist_ema20_mnq'] - e20_cl
    f['ema20_dist_spread_btc_cl']  = e20_mnq - e20_cl

    # Alinhamentos binarios MA50
    ab50_mnq = mnq['above_sma50'].reindex(idx, method='ffill')
    ab50_cl  = cl['above_sma50'].reindex(idx, method='ffill')
    f['sma50_align_mnq_btc'] = f['above_sma50_mnq'] + ab50_mnq
    f['sma50_align_mnq_cl']  = f['above_sma50_mnq'] + ab50_cl
    f['sma50_align_btc_cl']  = ab50_mnq + ab50_cl

    # Alinhamentos binarios EMA20
    eb20_mnq = mnq['above_ema20'].reindex(idx, method='ffill')
    eb20_cl  = cl['above_ema20'].reindex(idx, method='ffill')
    f['ema20_align_mnq_btc'] = f['above_ema20_mnq'] + eb20_mnq
    f['ema20_align_mnq_cl']  = f['above_ema20_mnq'] + eb20_cl
    f['ema20_align_btc_cl']  = eb20_mnq + eb20_cl

    # DI spread
    f['di_spread_mnq'] = f['pdi_mnq'] - f['mdi_mnq']
    f['di_spread_btc'] = pdi_mnq - mdi_mnq
    ...
    primary_high = btc['high']
    primary_low  = btc['low']
    primary_close = btc['close']
...
def plot_importance(model):
    ...
    ax.set_title('Feature Importance (Gain) - BTC XGBoost')
    plt.tight_layout()
