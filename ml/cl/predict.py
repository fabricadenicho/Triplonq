"""
Busca dados atuais, computa features (CL primario) e retorna predicao ML.
...
SYMS = {'cl': 'CL=F', 'mnq': 'MNQ=F', 'btc': 'BTC-USD'}
...
def main():
    ...
    try:
        cl_df  = fetch('CL=F')
        mnq_df = fetch('MNQ=F')
        cl_long = fetch_long('CL=F')
        btc_df = fetch('BTC-USD')

        if cl_df is None or mnq_df is None or btc_df is None:
        ...
        # Monta vetor de features (CL primario)
        r = {}
        ...
        r['pdi_cl']       = last(cl,  'pdi')
        r['mdi_cl']       = last(cl,  'mdi')
        r['div_mnq']      = r['rsi_cl'] - r['rsi_mnq']
        r['div_btc']      = r['rsi_cl'] - r['rsi_btc']
        r['ret1_cl']      = last(cl,  'ret1')
        r['ret4_cl']      = last(cl,  'ret4')
        r['ret8_cl']      = last(cl,  'ret8')
        r['vol_cl']       = last(cl,  'vol')
        r['bb_cl']        = last(cl,  'bb_w')
        r['ret1_mnq']     = last(mnq, 'ret1')
        r['ret4_mnq']     = last(mnq, 'ret4')
        r['ret1_btc']     = last(btc, 'ret1')
        r['ret4_btc']     = last(btc, 'ret4')
        r['hour']         = int(cl.index[-1].hour)
        r['dow']          = int(cl.index[-1].dayofweek)
        r['dadx_cl']      = (last(cl, 'adx') - float(cl['adx'].iloc[-3])) if len(cl) >= 3 else 0.0
        ...
        r['sma50_slope_cl']  = last(cl, 'sma50_slope') or 0.0
        ...
        r.update(compute_key_levels_live(cl_long if cl_long is not None else cl_df))
