"""
stats_nq.py — Estatisticas comportamentais do NQ/MNQ.
Analisa padroes de price action para day/swing traders.
Saida: JSON para stats.html

Uso: python ml/stats_nq.py
"""
...
def compute():
    ...
    if df_1h is None or df_d is None:
        return {'error': 'falha ao baixar dados'}

    # Rollover filter
    df_1h = df_1h[df_1h['close'].pct_change().abs() < 0.015].copy()
    df_d  = df_d[df_d['close'].pct_change().abs() < 0.05].copy()
    ...
    # ══════════════════════════════════════════════════════════════════════════
    # 1. BARRA DE 4H — comportamento geral
    # ══════════════════════════════════════════════════════════════════════════
    f = df_4h.copy()
    ...
    red4   = f[f['bull'] == False]
    gap_up = f[f['gap_pct'] >  0.05]
    gap_dn = f[f['gap_pct'] < -0.05]
    ...
    out['barra_4h'] = {
        ...
        'n_gap_up':                 len(gap_up),
        'n_gap_dn':                 len(gap_dn),
        'pct_gap_up_continua':      pct(gap_up['bull']) if len(gap_up) > 5 else None,
        'pct_gap_dn_continua':      round(100 - pct(gap_dn['bull']), 1) if len(gap_dn) > 5 else None,
        # Consecutivos 4H
        ...
        # Distribuicao fechamento vs open
        'dist_body': {
        ...
        # Extensao apos open (em barras bull: quanto vai contra antes de subir)
    ...
    out['ldh_ldl'] = {
        ...
        # Posicao do fechamento no range do dia
    ...
    out['monday'] = {
        ...
        'pct_2a_verde_sem_verde':     pct(mon_up['week_bull']) if len(mon_up) > 5 else None,
        'pct_2a_vermelha_sem_verm':   round(100 - pct(mon_dn['week_bull']), 1) if len(mon_dn) > 5 else None,
    }

    # ══════════════════════════════════════════════════════════════════════════
    # 4. PRIMEIRO CANDLE 4H → PROXIMO 4H E DIA
    # ══════════════════════════════════════════════════════════════════════════
    ...
    # ══════════════════════════════════════════════════════════════════════════
    # 5. ORB — Primeira hora RTH (13:00-14:00 UTC ≈ 9:00-10:00 ET)
    # ══════════════════════════════════════════════════════════════════════════
    h1 = df_1h.copy()
    ...
    # 7. POR DIA DA SEMANA
    # ══════════════════════════════════════════════════════════════════════════
    dow_names = ['Segunda', 'Terca', 'Quarta', 'Quinta', 'Sexta']
    dow_stats = []
    ...
    #     O usuario quer saber: depois que o 4H abre acima do close anterior,
    #     em quantos % fecha abaixo do open (reversao) e quanto fica acima
    # ══════════════════════════════════════════════════════════════════════════
    f['prev_close'] = f['close'].shift(1)
    f['abre_acima'] = f['open'] > f['prev_close']  # gap up
    f['abre_abaixo'] = f['open'] < f['prev_close'] # gap down

    above = f[f['abre_acima']].copy()
    below = f[f['abre_abaixo']].copy()

    # Buckets de fechamento relativo ao open para quem abriu acima
    ...
    out['abertura_4h'] = {
        ...
        # Se abre acima: fecha acima do open (continua) ou abaixo (reverte)?
        ...
        # Se abre abaixo: fecha abaixo do open (continua) ou acima (reverte)?
        ...
        # Distribuicao de fechamento (gap up)
